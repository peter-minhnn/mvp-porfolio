...
6. **Load only the relevant references**:
   - `references/codebase-map.md` - 2 mods, 5 entrypoints
   - `references/testing-map.md` - 0 test associations
   - `references/dependencies.md` - 3 dependencies
   - `references/public-api.md` - 3 risk items
...
**Quick-start search examples (from this codebase):**
   - `pnpm exec mp-sentinel indexing --find-import next --index-format json` - top dependency (used by 3 files)
...
|---|---|
| `src/app/` | architecture, codebase-map |
...
**Indexed Files:** 5
**Import Edges (graph):** 0
**Languages:** javascript (2), tsx (2), typescript (1)
**Key Scripts:** `pnpm run build`, `pnpm run dev`, `pnpm run start`, `pnpm run lint`
...
- tsconfig defines path alias(es) `@/*`; prefer them over deep relative imports when touching import-heavy files.
...
- **Current codebase P50:** 1 lines
- **Current codebase P95:** 20 lines
- **Current max file:** 20 lines
